...
def build_data(cfg):
    ...
    #data = data.head(cfg.fit_params['batch_size'] * 800)
...
def build_model(cfg, use_model, train_sequence_length, predict_sequence_length=288, target_aggs=1, short_feature_nums=10, long_feature_nums=1):
    inputs = (
        Input([1]),
...
def run_train(cfg):   
    ...
    history = trainer.train(train_data_loader, valid_dataset=valid_data_loader, **cfg.fit_params)
...
def objective(trial):
    ...
    bs = trial.suggest_float('batch_size', 5, 10)

    cfg.fit_params = {
        'n_epochs': n_epochs,
        'batch_size': int(2**int(bs)),
    ...
    cfg.custom_model_params['ffn_dropout'] = trial.suggest_float('ffn_dropout', 0.0, 0.5)
    cfg.custom_model_params['attention_dropout'] = trial.suggest_float('attention_dropout', 0.0, 0.5)
    rmse_error =  run_train(cfg)
    return sum(rmse_error) / len(rmse_error)
